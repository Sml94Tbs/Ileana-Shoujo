...
## L'instruction init offset fait exécuter les instructions d'initialisation de ce fichier tavant les instructions init des autres fichiers.
init offset = -2

## Appelé gui.init réinitialise les styles à leurs valeurs par défaut et initialise la largeur et la hauteur du jeu.
init python:
    gui.init(1920, 1080)

## Active la vérification de propriétés invalides ou instables dans les screens et transforms
...
## La petite couleur est utilisé pour les textes courts qui nécessitent d’être assombris ou éclairés pour obtenir le même effet.
define gui.idle_small_color = '#aaaaaa'

## Cette couleur est utilisée pour les boutons et les barres qui sont survolées.
define gui.hover_color = '#66c1e0'

## Cette couleur est utilisé pour le texte d’un bouton sélectionné, mais qui n’a pas le focus. Un bouton est sélectionné s’il est sur l’écran actuel ou si c’est la valeur de préférence.
define gui.selected_color = '#ffffff'

## La couleur utilisée pour le texte d’un bouton qui ne peut pas être sélectionné.
define gui.insensitive_color = '#8888887f'

## Couleurs utilisées pour les portions de barres qui ne sont pas remplies. Elles ne sont pas utilisées directement, mais quand les fichiers d’images sont régénérés.
...
## Dialogue ####################################################################
##
## Ces variables contrôlent comment les dialogues sont affichés une ligne à la fois.

## La hauteur de la fenêtre contenant les dialogues.
define gui.textbox_height = 278

## L’emplacement vertical de la zone de texte à l’écran. 0.0 pour le haut, 0.5 pour le centre et 1.0 pour le bas.
...
## Boutons #####################################################################
##
## Ces variables, ainsi que les fichiers d’image dans gui/button, contrôlent la façon d’afficher les boutons et leur aspect.
...
## L'alignement horizontal du texte des boutons. (0.0 est à gauche, 0.5 est au centre, 1.0 est à droite).
define gui.button_text_xalign = 0.0


## Ces variables surchargent les paramètres par défaut pour différents types de boutons. Veuillez consulter la documentation de l’interface de jeu (GUI) pour les types de boutons disponibles et leurs usages.
##
...
## Vous pouvez également ajouter vos propres personnalisations en ajoutant des variables correctement nommées. Par exemple, vous pouvez décommanter la ligne suivante pour personnaliser la largeur du bouton de navigation.
...
##
## Les boutons pour les choix (Choice buttons) sont utilisés dans le jeu pour permettre au joueur de choisir telle ou telle action, tel ou tel dialogue.
...
##
## Un bouton d’emplacement de fichier est un type spécial de bouton. Il contient une vignette et un texte décrivant le contenu de la sauvegarde présente dans l’emplacement. Un emplacement de sauvegarde utilise une image dans gui/button, comme les autres types de bouton.
...
## La largeur et la hauteur des vignettes de sauvegarde utilisée pour les emplacements de sauvegarde.
define config.thumbnail_width = 384
define config.thumbnail_height = 216

## Le nombre de colonnes et de lignes pour la grille des emplacements de sauvegarde.
...
## Cadres ######################################################################
##
## Ces variables contrôlent le look des cadres qui peuvent contenir les composants de l’interface utilisateur quand un overlay ou une fenêtre ne sont pas présents.
...
##
## Ceux-ci contrôlent le look et la taille des barres, des ascenseurs et des curseurs.
##
## Le GUI par défaut utilise uniquement des sliders et des barres de scrolling verticales. Toutes les autres barres ne sont utilisées que dans des screens écrits par l'utilisateur.

## La hauteur des barres, des ascenseurs et des curseurs horizontaux. La largeur des barres, des ascenseurs et des curseurs verticaux.
define gui.bar_size = 38
define gui.scrollbar_size = 18
define gui.slider_size = 38

## « True » (Vrai)  si les images de barres doivent être en mosaïques. « False »(Faux) si elles doivent être mise à l'échelle (étirement).
...
## La hauteur de l’écran historique ou « None » pour calculer la hauteur au prix d’une légère perte de performance.
...
## Ceci contrôle où un saut de ligne est autorisé. La valeur par défaut convient à la plupart des langues. Une liste des valeurs disponible peut être trouvée sur https://www.renpy.org/doc/html/style_properties.html#style-property-language
...
init python:
    ...
    ## Ceci augmente la taille des boutons d’accès rapide pour les rendre plus accessibles sur les tablettes et les téléphones.
    @gui.variant
    def touch():

        gui.quick_button_borders = Borders(60, 21, 60, 0)

    ## Ceci change la taille et l’espacement de différents élements de la GUI pour s’assurer qu’ils soient visibles sur les téléphones.
    @gui.variant
    def small():
